...
- `GameplayLoopController.TryRecoverLootDrop` is the authoritative between-expedition transaction. It validates Expansion phase and drop availability, claims through `NPCTraversal.TryClaimRecoverableLoot`, transfers item snapshots into dungeon storage, records the accepted recovery by source drop ID, and never mutates Dread.
...
3. With no build/removal tool active, click a physical bag directly in the Game View. Verify the bag, POI, and authoritative recoverable record disappear; dungeon-storage item/value totals increase by exactly the displayed amounts; and Dread does not change. Confirm clicking UI or an Editor tool does not recover a bag.
